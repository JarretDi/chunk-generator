...
public:
    ...
    void ProcessKeyboard(Camera_Movement direction, float deltaTime)
    {
        ...
        if (direction == FORWARD)
            Position += glm::normalize(vec3(Front.x, 0, Front.z)) * velocity; // so you cant move up/down
        if (direction == BACKWARD)
            Position -= glm::normalize(vec3(Front.x, 0, Front.z)) * velocity;
        if (direction == LEFT)
            Position -= Right * velocity;
        if (direction == RIGHT)
            Position += Right * velocity;
        if (direction == UP)
            Position += WorldUp * (velocity / 2.0f);
        if (direction == DOWN)
            Position -= WorldUp * (velocity / 2.0f);
